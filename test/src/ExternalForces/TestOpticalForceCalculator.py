...
class TestOpticalForceCalculator(unittest.TestCase):
    def setUp(self):
        ...
        self.initial_conditions, self.connectivity_matrix = MF.mesh_square(
            1, 1, 0.02, self.params
        )
        ...
        self.PS.calculate_correct_masses(1e-3, 1000)  # [m] thickness  # [kg/m3] density
        ...
        self.LB_flat = LaserBeam(
            lambda x, y: np.ones(x.shape) * I_0, lambda x, y: [0, 1]
        )
            ...
            with self.subTest(i=i):
                ...
                for particle in self.PS.particles:
                    ...
                    particle.axicon_angle = Rotation.from_euler(
                        "y", directing_angle, degrees=True
                    ).as_matrix()
    ...
    def test_rotate_mesh(self):
        OpticalForces = self.OpticalForces
        mesh = np.meshgrid(np.linspace(0, 1, 6), np.linspace(0, 1, 6))
    ...
    def test_stability_unit_mesh_uniform_beam(self):
        ...
        with self.subTest(i=2):
            ...
            force_horizontal = (
                expected_force * np.sin(np.deg2rad(5)) * np.cos(np.deg2rad(5))
            )
        ...
        with self.subTest(i=3):
            ...
            d_force_vertical_d_deg = (
                -expected_force * (1 - np.cos(np.deg2rad(5)) ** 2) / 5
            )
    ...
    def test_stability_unit_mesh_bounded_uniform_beam(self):
        ...
        with self.subTest(i=7):
            ...
            force_vertical = expected_force * (
                1 - 0.1
            )  # displacement 0.1 out of width 1
        ...
        with self.subTest(i=9):
            ...
            moment_arm = (
                0.1 / 2 + 0.4
            )  # distance to center of strip + distance strip+COM
    ...
    def test_calculate_restoring_forces(self):
        ...
        with self.subTest(i=1):
            self.assertTrue(
                np.allclose(np.abs(net_moments_pos), np.abs(net_moments_neg))
            )
