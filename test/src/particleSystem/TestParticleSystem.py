...
class TestParticleSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.PS = ParticleSystem(*mesh, self.params, clean_particles=False)
    ...
    def test_constraint_point(self):
        ...
        connectivity_matrix = [[0, 1, 1, 1], [1, 2, 1, 1]]
    ...
    def test_constraint_line(self):
        ...
        connectivity_matrix = [[0, 1, 1, 1], [0, 2, 1, 1], [1, 2, 1, 1]]
        PS = ParticleSystem(connectivity_matrix, initial_values, self.params)
...
class TestParticleSystemReactionForces(unittest.TestCase):
    def setUp(self):
        ...
        for j in range(3):  # Loop for each arm
            ...
            for i in range(n_points_per_arm):  # loop for each point
                ...
                if i == n_segments_per_arm:
                    initial_conditions.append(
                        [
                            [x_coordinates[i], y_coordinates[i], 0],
                            np.zeros(3),
                    ...
                    if not (
                        center_added and x_coordinates[i] == 0 and y_coordinates[i] == 0
                    ):
                        initial_conditions.append(
                            [
                                [x_coordinates[i], y_coordinates[i], 0],
            ...
            new_coordinates = rotation_matrix.dot(
                np.vstack([x_coordinates, y_coordinates, np.zeros(x_coordinates.shape)])
            ).T
            x_coordinates = new_coordinates[:, 0]
            y_coordinates = new_coordinates[:, 1]

        self.PS = ParticleSystem(
            connectivity_matrix, initial_conditions, self.params, clean_particles=False
        )
        self.PS.particles[0].set_m(self.params["proof_mass"])

    def test_find_reaction_forces(self):
        expected_vertical_force = (len(self.PS.particles) - 4) * self.params[
            "m_particle"
        ]
