...
class Particle(SystemObject):
        ...
        if self.__fixed:
            ...
            if self.__constraint_type not in ["point", "line", "plane"]:
                raise AttributeError(
                    f"Incorrect constraint type set, expected"
                    f" line or plane, got "
            ...
            try:
                ...
                raise AttributeError(
                    f"Particle set as 'fixed' but constraint "
    ...
    def constraint_projection(self):
        if np.sum(self.__constraint == 0) == 3:
            ...
            normalised_constraint = self.__constraint / np.linalg.norm(
                self.__constraint
            )
            if self.__constraint_type == "plane":
                projection_matrix = np.eye(3) - np.outer(
                    normalised_constraint, normalised_constraint
                )
                self.constraint_projection_matrix = projection_matrix
            elif self.__constraint_type == "line":
                projection_matrix = np.outer(
                    normalised_constraint, normalised_constraint
                )
                self.constraint_projection_matrix = projection_matrix
    ...
    def update_pos(self, new_pos: npt.ArrayLike):
        if not self.__fixed:
            ...
            self.__x += self.constraint_projection_matrix.dot(
                np.array(new_pos) - self.__x
            )
    ...
    def update_vel(self, new_vel: npt.ArrayLike):
        if not self.__fixed:
            ...
            self.__v += self.constraint_projection_matrix.dot(
                np.array(new_vel) - self.__v
            )
