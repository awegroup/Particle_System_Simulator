...
def mesh_square(length, width, mesh_edge_length, params=params):
    ...
    mesh = np.meshgrid(np.linspace(0, length, n_long), np.linspace(0, width, n_wide))
...
def mesh_square_concentric(length, mesh_edge_length, params=params, fix_outer=False):
    ...
    for i, node in enumerate(initial_conditions):  # adding connections in x-axis
        if (
            (i + 1) % (n_long) and abs(node[0][0]) <= abs(node[0][1]) and node[0][0] < 0
        ):  # Using modulus operator to exclude the nodes at the end of a row
            connections.append([i, i + 1, params["k"], params["c"]])
        elif (
            (i + 1) % (n_long) and abs(node[0][0]) < abs(node[0][1]) and node[0][0] >= 0
...
def mesh_airbag_square_cross(
    ...
    initial_conditions, connections = meshfunct(length, width, mesh_edge_length, params)
    ...
    for particle in initial_conditions:
        ...
        print(f"particle: {particle}, type: {type(particle)}")
        ...
        elif (particle[0][0] == length and particle[0][1] > 0) or (
...
def mesh_phc_square_cross(
    ...
    params["k_d"] = (
        params["G"] * params["thickness"] * x_length / (y_length * n_wide / np.sqrt(2))
    )
    ...
    params["k_x"] *= (
        params["k_x"]
        * n_long
        / (params["k_x"] * n_long + params["k_d"] * np.sqrt(2) * n_long)
    )
    params["k_y"] *= (
        params["k_y"]
        * n_wide
        / (params["k_y"] * n_wide + params["k_d"] * np.sqrt(2) * n_wide)
    )
    ...
    for xyz in xyz_coordinates:
        if fix_outer and (
            xyz[0] == 0 or xyz[0] == length or xyz[1] == 0 or xyz[1] == width
        ):
            initial_conditions.append([xyz, np.zeros(3), params["m_segment"], True])
        else:
            initial_conditions.append([xyz, np.zeros(3), params["m_segment"], False])
...
def mesh_circle_square_cross(
    radius, mesh_edge_length, params=params, fix_outer=False, edge=0
):
    n_wide = int(radius / mesh_edge_length + 1)
    n_long = n_wide

    mesh = np.meshgrid(
        np.linspace(-radius, radius, n_long), np.linspace(-radius, radius, n_wide)
    )
    ...
    if fix_outer:
        ...
        inner = (
            xy_coordinates[:, 0] ** 2 + xy_coordinates[:, 1] ** 2
            <= (radius - edge) ** 2
        )
...
def mesh_round_phc_square_cross(
    radius,
    mesh_edge_length=1 / 10,
    params=None,
    noncompressive=False,
    ...
    xyz_coordinates = np.column_stack(
        (filtered_xy_coordinates, np.zeros(len(filtered_xy_coordinates)))
    )
    initial_conditions = [
        [xyz, np.zeros(3), params["m_segment"], False] for xyz in xyz_coordinates
    ]
    ...
    for i, cond_i in enumerate(initial_conditions):
        for j, cond_j in enumerate(initial_conditions[i + 1 :], start=i + 1):
            ...
            if distance <= mesh_edge_length:  # Direct neighbors
                k_value = (
                    params["k_x"] if cond_i[0][1] == cond_j[0][1] else params["k_y"]
                )
                ...
                distance <= mesh_edge_length * np.sqrt(2) and params["G"] != 0
            ):  # Diagonal connections only if G != 0
...
if __name__ == "__main__":
    ...
    initial_conditions, connections = mesh_rotate_and_trim(
        initial_conditions, connections, 45 / 2
    )
    PS = ParticleSystem(connections, initial_conditions, params)
    ...
    ax.set_title(
        (
            mesh_square_cross.__name__,
            mesh_rotate_and_trim.__name__,
            ps_fix_opposite_boundaries_x.__name__,
            ps_stretch_in_x.__name__,
        )
    )
