...
class AnalyseMaterial():
    ...
    def stiffness_sweep(self, chirality = False):
        ...
        for i, ratio in enumerate(stiffness_ratio_range):
            ...
            print(f'That took {dt:.1f}s, est. time remaining {t_rest//60:.0f}m {t_rest%60:.1f}s')
            
            last_time = curr_time
            for item in result.items():
                testdata.append((ratio, item[0], item[1][0],item[1][1]))
            
        dtype=[('Stiffness_ratio', 'f4'),
               ('Strain', 'f4'), 
               ('Reaction', 'f4'), 
               ('Poisson', 'f4')]
    ...
    def run_testcase(self, case_id):
        ...
        n_segments = 20
        initial_conditions, connections = MF.mesh_square_cross(1,1,1/n_segments,params)
        ...
        title = f"MaterialTest, {params['k']=:.4f},{params['k_d']=:.4f},{strain=:.4f}".strip('params')
        ax.set_title(title)
        fig.tight_layout()
