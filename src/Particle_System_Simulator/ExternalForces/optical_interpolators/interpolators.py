...
def create_interpolator_specular() -> Callable:
    ...
    polar_in, azimuth_in, polarization_in = np.meshgrid(
        polar_in, azimuth_in, polarization_in
    )
            ...
            if np.any(np.isnan(v)):
                for i, line in enumerate(v):
                    if np.any(np.isnan(line)):
                        logging.warning(
                            "Interpolation error resulting in nan values for input "
                            + str(coordinates[i, :])
                        )
            v = np.nan_to_num(v)
...
def check_interpolator(interp, coordinates, ax=None):
    ...
    theta_out, phi_out, pol_out = wrap_spherical_coordinates(
        *[np.array(i, dtype=float) for i in coordinates]
    )
    ...
    ax.quiver(
        0, 0, 0, -in_x, -in_y, -in_z, label="incident ray", color="y", pivot="tip"
    )
    ax.quiver(
        0, 0, 0, out_x, out_y, out_z, label="net scattered ray", color="r", pivot="tail"
    )
    ax.legend()
...
if __name__ == "__main__":
    ...
    ax.quiver(
        0, 0, 0, s_x, s_y, s_z, label="net scattered ray", color="r", pivot="tail"
    )
    ax.legend()
