...
def cm_and_ic(mesh_file, m, E, c):
        ...
        if line.startswith("$Elements"):  # retrieve nodal connections
            ...
            for j in range(1, total_lines):
                if len(lines[i + j].split()) != 4:
                    ...
                    connections.append(
                        [int(connection[i]) for i in range(1, len(connection))]
                    )
    ...
    connections = (
        np.array([connection for connection in connections if len(connection) == 4]) - 1
    )
...
c_matrix, init_cond, element_param, params["n"], element_list = cm_and_ic(
    filename, 1, E_t, params["c"]
)
