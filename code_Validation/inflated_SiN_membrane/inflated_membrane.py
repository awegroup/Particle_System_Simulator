...
curve_fitting_parameters = {
    ...
    "205": {"A1/s": 3.426e-02, "A2/s": 1.259e-04, "A1/r": 2.014e-02, "A2/r": 7.291e-05},
}

curve_fitting_parameters_analytical = {
    "125": {"A1/s": 1.304e-02, "A2/s": 1.582e-04},
    "205": {"A1/s": 3.325e-02, "A2/s": 1.383e-04},
}

material_properties = {
    "125": {"E [GPa]": 79, "Sigma_0 [MPa]": 5.1},  # +/- 4  # +/- 1.5
    "205": {"E [GPa]": 151, "Sigma_0 [MPa]": 29},  # +/- 6  # +/- 3
    "rho": 3.17e3,  # [kg/m3]
}

# Misread the paper maybe? there are two sets and which is the right one is unclear
material_properties_alternative = {
    "125": {"E [GPa]": 82, "Sigma_0 [MPa]": 4.3},
    "205": {"E [GPa]": 146, "Sigma_0 [MPa]": 32},
    "rho": 3.17e3,  # [kg/m3]
}
...
params_square_high_t["ratio_k_k_d"] = (
    0.3  # Coefficient determined for this mesh shape to provide poissons ratio of 0.25
)

params_rectangle_high_t = params.copy()
params_rectangle_high_t["testcase_name"] = "Rectangluar Membrane, T=205"
params_rectangle_high_t["ratio_k_k_d"] = (
    0.3  # Coefficient determined for this mesh shape to provide poissons ratio of 0.25
)
...
params_rectangle_high_t["convergence_threshold"] *= (
    9000 / 1900
)  # Adjust convergence crit for larger system (more mass = more E_kin)

params_square_low_t = params_square_high_t.copy()
params_square_low_t["testcase_name"] = "Square Membrane, T=125"
params_square_low_t["ratio_k_k_d"] = (
    0.2755  # Coefficient determined for this mesh shape to provide poissons ratio of 0.23
)
...
params_rectangle_low_t["ratio_k_k_d"] = (
    0.2755  # Coefficient determined for this mesh shape to provide poissons ratio of 0.25
)
params_rectangle_low_t["temperature"] = "125"
params_rectangle_low_t["thickness"] = 680e-9

testcases = [
    params_square_high_t,
    params_rectangle_high_t,
    params_square_low_t,
    params_rectangle_low_t,
]
...
def setup_testcase(params=params, n_segments=20):
    ...
    params["k"] = (
        material_properties[params["temperature"]]["E [GPa]"]
        * 1e9
        * params["thickness"]
        * n_segments
    ...
    print(
        f"Applied pre-stress is {sigma_0_center/1e6:.2f} [MPa] in the center, {sigma_0_mean/1e6:.2f} on average."
    )
