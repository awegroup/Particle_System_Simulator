...
def plot(psystem: ParticleSystem, ps2: ParticleSystem, ps3: ParticleSystem):
    ...
    plt.legend([f"PS simulation, dt = 0.01 s", f"PS simulation, dt = {input.params['dt']} s", f"PS simulation, dt = 1 s"
                ,  "Exact solution"])
    plt.grid()
    #
    ...
    plt.title("Normalized errors at the first simulation step and timestep squared curve")
